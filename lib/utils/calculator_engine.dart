...
class CalculatorEngine {
  /// Calculates standard Malaysian commercial housing loan monthly installment,
  /// tiered SPA stamp duty, legal fees, valuation, and insurance.
  static MortgageResult calculateMortgage({
    ...
    final double loanAmount = max(0.0, propertyPrice - downPaymentAmount);

    // Amortized monthly installment formula: M = L * [ i(1+i)^n ] / [ (1+i)^n - 1 ]
    ...
    if (monthlyRate > 0 && totalMonths > 0) {
      ...
      monthlyInstallment = (loanAmount * (monthlyRate * compound)) / (compound - 1);
    ...
    if (propertyPrice > 1000000) {
      ...
      standardStampDuty = (100000 * 0.01) + ((propertyPrice - 100000) * 0.02);
    } else {
      standardStampDuty = propertyPrice * 0.01;
    }

    // First-Time Home Buyer exemption:
    // 100% exemption <= RM500k, 75% remission from RM500k - RM1M
    double stampDuty = standardStampDuty;
    if (isFirstHomeBuyer) {
      if (propertyPrice <= 500000) {
        stampDuty = 0;
      } else if (propertyPrice <= 1000000) {
        stampDuty = standardStampDuty * 0.25;
      }
    }

    // Scale of Legal Fees (Solicitors' Remuneration Order ~1.1% of value, min RM2,500)
    final double legalFees = max(2500.0, propertyPrice * 0.011);

    // Bank Valuation Fee estimation (~0.3% of value, min RM1,000)
    final double valuationFee = max(1000.0, propertyPrice * 0.003);

    // Base Total Initial Cash Required = Downpayment + Stamp Duty + Legal Fees + Valuation
    final double totalUpfront =
        downPaymentAmount + stampDuty + legalFees + valuationFee;

    // Recommended Min. Net Household Income (assume 60% DSR)
    final double recommendedIncome = monthlyInstallment / 0.60;
    ...
    final double autoMrtt = loanAmount > 0 ? loanAmount * mrttRate : 0.0;
    final double mrttEstimate = hasMrtt
        ? (customMrttAmount != null && customMrttAmount > 0 ? customMrttAmount : autoMrtt)
        : 0.0;

    // Houseowner / Fire Insurance (Tarif standard BNM ~0.115% per annum)
    final bool hasFire = includeInsurance && includeFireInsurance;
    final double fireInsuranceAnnual =
        hasFire && propertyPrice > 0 ? propertyPrice * 0.00115 : 0.0;
    final double fireInsuranceMonthly = fireInsuranceAnnual / 12.0;

    // Effective loan amount if MRTT is financed into mortgage
    final bool isInsuranceFinanced = hasMrtt && financeMrtt && mrttEstimate > 0;
    final double effectiveLoanAmount =
        isInsuranceFinanced ? loanAmount + mrttEstimate : loanAmount;
    ...
    if (isInsuranceFinanced && monthlyRate > 0 && totalMonths > 0) {
      ...
      monthlyInstallmentWithInsurance =
          (effectiveLoanAmount * (monthlyRate * compound)) / (compound - 1);
    }
    final double monthlyMrttDelta = isInsuranceFinanced
        ? max(0.0, monthlyInstallmentWithInsurance - monthlyInstallment)
        : 0.0;
    final double effectiveMonthly = isInsuranceFinanced ? monthlyInstallmentWithInsurance : monthlyInstallment;
    final double totalPayment = effectiveMonthly * totalMonths;
    final double totalInterest = max(0.0, totalPayment - effectiveLoanAmount);

    // Total Upfront Cash with insurance considerations
    final double upfrontInsuranceCost =
        (isInsuranceFinanced ? 0.0 : mrttEstimate) + fireInsuranceAnnual;
